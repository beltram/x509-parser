...
fn test_crl_parse() {
    match parse_x509_crl(CRL_DER) {
        Ok((e, cert)) => {
            ...
            let tu: DateTime<Utc> = DateTime::from_utc(NaiveDateTime::from_timestamp(this_update.timestamp(), 0), Utc);
            let nu: DateTime<Utc> = DateTime::from_utc(NaiveDateTime::from_timestamp(next_update.timestamp(), 0), Utc);
